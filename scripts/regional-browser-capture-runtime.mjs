...
export async function collectQueryPage({
  ...
  const audit = await tab.playwright.evaluate(
    ({ idParam: identity, pageParam: pagination, page: current, pageStep: step, titleSelector: title, closestSelector: closest, linkSelector: selectedLinks, identityPattern: identityRegex, identityAttribute: identityAttr }) => {
    ...
    { idParam, pageParam, page: paginationValue, pageStep, titleSelector, closestSelector, linkSelector, identityPattern, identityAttribute },
  );
  const discovered = [];
  const seen = new Set();
  ...
  const discovery = await postDiscovery(endpoint, token, {
    source_id: sourceId,
    page,
  ...
  const pending = new Set(discovery.pending_ids);
  const listClosed = [];
    ...
    for (const item of pendingItems.slice(start, start + 3)) {
      ...
      items.push({
        ...
        request_identity: detailPost
          ? `${detailPost.identityField}=${item.external_id}&${Object.entries(detailPost.fields).map(([key, value]) => `${key}=${value}`).join("&")}`
          : null,
        detail,
      });
    }
...
async function extractDetail(tab, expectedTitle, detailTitleSelector) {
  const extracted = await tab.playwright.evaluate((selector) => {
    const squash = (value) => String(value ?? "").replace(/\s+/g, " ").trim();
    ...
    return {
      ...
      pairs,
      actualTitle: selector ? squash(document.querySelector(selector)?.textContent) : null,
    };
  }, detailTitleSelector);
...
function buildDetail(title, extracted) {
