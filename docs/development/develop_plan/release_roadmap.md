...
- Backend가 `keyword`, 지역, 연령, 카테고리, 신청 상태와 pagination을
  서버에서 처리한다.
...
- 기본 사용자 검색은 신청 가능 정책을 우선하고 마감·예정 정책의 표시 규칙을
  구분한다.
- 자연어 입력은 우선 결정적인 한국어 조건 추출로 `keyword`, 지역, 연령과
  카테고리를 만든다. LLM·벡터 검색은 `v0.1.0`의 필수 조건이 아니다.
- Frontend는 목록 100건 전체를 받은 뒤 로컬에서만 검색하지 않고 Backend
  query와 pagination 결과를 사용한다.
...
| 검색이 keyword·기본 필터 중심 | 자연어 조건 추출과 서버 검색을 `v0.1.0` 필수로 포함 | 현재 사용자 기대 시나리오를 client-only 문자열 검색으로 만족할 수 없음 |
