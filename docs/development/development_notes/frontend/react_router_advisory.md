...
- 현재 도달 불가 판정만으로 advisory를 종료하거나 위험 수용을 확정하지
  않는다.
- npm의 downgrade 제안을 자동 적용하지 않는다. 안전 버전과 Router API
  호환성은 F1에서 검토한다.
...
- F1에서 patched 8.3.0 upgrade, npm이 제안한 7.11.0 downgrade 또는 현재
  구성의 한시적 위험 수용을 호환성 근거와 함께 결정한다.
- F2에서 F1 결정에 따른 manifest·lockfile 또는 보호 조건을 반영한다.
