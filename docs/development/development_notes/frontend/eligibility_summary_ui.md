...
- 현재 Slice: FE7-01~04 completed

## 목적

정책 상세 `핵심 신청 조건` 카드(FE7-01~04)를 Integration 08 W4-G0 proposal
DTO·Mock-first detail envelope에 맞춰 구현한다.

## Forest 범위

이 기록은 Frontend 07 Slice 구현·검증 결과를 누적한다. W4-G0 승인 전
`eligibility_summary` nested 구조는 proposal이며 Real API·Browser E2E(FE7-05)는
...
Real API detail에 summary가 없으면 카드 empty state를 표시한다(FE7-05 재검증).
...
- `npm run test:e2e`: **미실행** (FE7-05 범위)
