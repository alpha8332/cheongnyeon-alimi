...
- 실제 저장소 상태와 계획이 다를 때 임의로 구조를 바꾸지 않는 원칙을
  추가했다.
- 2026-07-26 후속 보완에서 Frontend·Backend·Data가 manifest·lockfile과 실행
  방법을 제공하고, Dockerfile·Compose·배포는 병합 후 통합·배포 담당이
  수행하도록 책임을 분리했다.
...
- 2026-07-26 후속 보완에서 각 영역 산출물이 `develop`에 병합된 뒤
  통합·배포 Forest에서 세 컨테이너와 Compose를 구성하는 순서를 명시했다.
...
- 2026-07-26 후속 보완에서 담당 영역을 재귀 탐색하고, 허용 영역·동일 영역
  대응과 계획·기록의 README 및 `docs/index.md` 등록을 검사하도록 확장했다.
...
- 2026-07-26 후속 보완 단위 테스트 9개와 실제 저장소 문서 검증이 통과했다.
