...
| --- | --- | --- | --- | --- | --- |
| `INT-02-D0-FE` | canonical Seed Mock과 공개 Policy DTO의 Frontend 소비 승인 | `review-pending` | Frontend | FE 2A의 공개 `PolicyDto` 변환과 provenance·invalid 비노출 소비 테스트·lint·build 실행 증거 확인 | [Fixture·Seed 계약](data/fixture_seed_contract.md), [Policy API 계약](api/policies.md), [Frontend 개발 기록](development/development_notes/frontend/policy_discovery.md) |
| `INT-02-D3-FE` | 실제 PostgreSQL 기반 Policy 목록·상세 API의 Frontend 소비 | `review-pending` | Frontend | FE 2A의 `/api/v1/policies`, 목록 envelope, 숫자 `id` 상세와 partial opt-in 소비 테스트·lint·build 실행 후 D6 기록 동기화 | [Policy API 계약](api/policies.md), [Frontend 개발 기록](development/development_notes/frontend/policy_discovery.md) |
