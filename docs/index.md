...
| `BE-POLICY-TIMESTAMP-ORDER` | 최초 insert에서 Importer `updated_at`이 뒤이어 실행되는 ORM Python default `created_at`보다 먼저 생성되는 순서의 허용 여부 결정 | `action-needed` | Backend, API 영향 검토 | Backend 03에서 두 시각의 생성 주체와 순서 불변식을 정하고 SQLite·PostgreSQL 회귀 및 문서 동기화 | [Policy Runtime Safety 계획](development/develop_plan/backend/03_policy_runtime_safety.md), [Policy DB 매핑](architecture/policy_database_mapping.md) |
