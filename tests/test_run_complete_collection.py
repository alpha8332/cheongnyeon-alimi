...
def test_queue_and_wait_allows_approved_youthcenter_source(
    monkeypatch, session_factory
):
    published = []

    def fake_wait(run_id, **kwargs):
        return _state(run_id, source_id="youthcenter-api")
    ...
    assert result["source_id"] == "youthcenter-api"
    assert published[0][1:] == ("youthcenter-api", 500, True)
